        ...
        <table class="w-full border-collapse border border-gray-300">
            ...
            <tbody>
                @foreach($this->getTotalPerParameter() as $param)
                    <tr>
                        <td class="border border-gray-300 px-4 py-2">{{ $param->kd_unsurikmtu }}</td>
                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $param->total_skor }}</td>

                        @php
                            $avgSkor = collect($this->getAveragePerParameter())->where('kd_unsurikmtu', $param->kd_unsurikmtu)->first();
                            $skm = collect($this->getSkmPerParameter())->where('kd_unsurikmtu', $param->kd_unsurikmtu)->first();
                        @endphp
